...
**Full support.** `core::error::Error` has been part of `core` since Rust
1.81, so implementing it — by hand, as shown above, or via a `no_std`-
compatible derive — works identically in `#![no_std]`. The one caveat is
`Box<dyn Error>`: `Box` needs the `alloc` crate and a
`#[global_allocator]`, so that specific pattern requires `alloc` even
though the `Error` trait itself does not.
