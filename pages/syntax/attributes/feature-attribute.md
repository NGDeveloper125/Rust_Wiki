...
`#![feature(...)]` itself works identically regardless of target — it's
a toolchain-level gate, unaffected by `std` vs `#![no_std]`. The one point
worth naming for embedded specifically: a handful of capabilities
embedded code has wanted for a long time — improvements to
`asm!`/`global_asm!` before they stabilized, certain const-generics
refinements, niche `core` APIs — spent real time as nightly-only
features, and embedded projects have, anecdotally, been slightly more
willing than typical application code to track nightly and accept
`#![feature(...)]`'s stability risk while waiting, simply because there's
often no stable workaround for a capability that reaches directly into
architecture-specific low-level code. This is a minor, project-by-project
pattern, not a rule — plenty of embedded crates target stable
exclusively, and the same "this can change without notice" caveat from
the classic explanation applies without exception here too.

## Usage examples (Embedded)

### A firmware crate tracking nightly for an inline-asm improvement

```
// Nightly-only — illustrative of the kind of low-level feature embedded
// code has sometimes tracked nightly for before stabilization.
#![feature(asm_const)]

use core::arch::asm;

unsafe fn delay_cycles<const N: u32>() {
    asm!("1: subs {0}, {0}, #1", "bne 1b", inout(reg) N => _); // <- uses the const generic directly inside asm!
}
```
