...
**Full support**, but this attribute is genuinely rare even within
embedded Rust — it matters to kernel-level and truly freestanding crates
that provide their own memory-copy primitives, not to ordinary
application-level `#![no_std]` firmware, which typically links against a
`compiler_builtins`-provided implementation of these routines without
ever needing `#![no_builtins]` itself.
